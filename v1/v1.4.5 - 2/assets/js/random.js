...
let _autoCount  = 0;
let _curLink    = null;
// Clamp preview height to usable viewport space on mobile
function _defaultPreviewH() {
  const vh = window.innerHeight || 600;
  const max = Math.floor(vh * 0.55); // max 55% of viewport
  return Math.min(Math.max(200, Storage.get('randomPreviewHeight', 360)), max);
}
let _previewH = _defaultPreviewH();
      ...
      <div class="random-scroll">
        ...
        <div class="r-preview-card hidden" id="r-preview-card">
          ...
          <div class="r-preview-iframe-wrap" id="r-iframe-wrap"
            style="height:${_defaultPreviewH()}px;min-height:200px;max-height:calc(100dvh - 200px);position:relative;background:var(--surface-2)">
...
// ── Domain chips ──────────────────────────────────────────
const DOMAIN_SHOW_LIMIT = 6; // show more/less threshold
...
function _refreshDomainUI() {
  ...
  const domains     = _allDomains;
  const showAll     = _domainExpanded || domains.length <= DOMAIN_SHOW_LIMIT;
  const visible     = showAll ? domains : domains.slice(0, DOMAIN_SHOW_LIMIT);
  const hiddenCount = domains.length - DOMAIN_SHOW_LIMIT;

  bar.innerHTML = visible.map(d => `
    <button class="r-domain-chip${_selDomains.includes(d)?' sel':''}" data-domain="${escapeHtml(d)}">
      ${escapeHtml(d)}
    </button>`).join('') +
    (domains.length > DOMAIN_SHOW_LIMIT ? `
      <button class="r-domain-chip r-domain-toggle" id="r-domain-more">
        ${showAll
          ? '<i class="fa-solid fa-chevron-up" style="font-size:9px"></i> Less'
          : `<i class="fa-solid fa-chevron-down" style="font-size:9px"></i> +${hiddenCount} more`}
      </button>` : '');

  bar.querySelectorAll('.r-domain-chip:not(.r-domain-toggle)').forEach(chip => {
    chip.addEventListener('click', () => {
  ...
  bar.querySelector('#r-domain-more')?.addEventListener('click', () => {
...
function _showPreview(page, link) {
  ...
  // ── Status indicator ─────────────────────────────────────
  // Small pill in bottom-right corner of the preview area that shows
  // the current preview strategy. Auto-hides after 2.5s.
  let _statusTimer;
  const setStatus = (text, icon = 'fa-circle-info') => {
    let pill = page.querySelector('#r-preview-status');
    if (!pill) {
      pill = document.createElement('div');
      pill.id = 'r-preview-status';
      pill.style.cssText = [
      ...
      page.querySelector('#r-iframe-wrap')?.appendChild(pill);
    }
    pill.innerHTML = `<i class="fa-solid ${icon}" style="font-size:9px"></i>${text}`;
    pill.style.opacity = '1';
    clearTimeout(_statusTimer);
    _statusTimer = setTimeout(() => { pill.style.opacity = '0'; }, 2500);
  };
  ...
  const showFavLayer = () => {
    ...
    setStatus('Icon only', 'fa-globe');
  };

  setStatus('Loading…', 'fa-spinner fa-spin');

  const tryOgImage = async () => {
    setStatus('Fetching thumbnail…', 'fa-image');
    try {
