  ...
  const items = [
    ...
    },
    {
      ...
      action: async () => {
        ...
        if (name) renameFolderAndSync(folder.id, name);
      },
    },
    {
      label:  'Open in Random',
      icon:   'fa-solid fa-shuffle',
      action: () => {
        // Pre-select this folder in random page
        window.Router?.go?.('random');
        setTimeout(() => document.dispatchEvent(new CustomEvent('linkivo:selectRandomFolder', { detail: { folderId: folder.id } })), 200);
      },
    },
    {
...
async function openFolder(folder) {
  if (folder.locked) {
    const ok = await unlockFolderWithSession(folder.id);
    if (!ok) return;
  }
  // Navigate via router
...
// ══════════════════════════════════════════════════════════
// v1.4.0 — SESSION PIN CACHE for locked folders
// ══════════════════════════════════════════════════════════
// Stores unlocked folder IDs for the session (cleared on page unload)
const _sessionUnlocked = new Set();

export function isSessionUnlocked(fid) {
  return _sessionUnlocked.has(fid);
}

export function setSessionUnlocked(fid) {
  _sessionUnlocked.add(fid);
  // Clear on page unload
  window.addEventListener('beforeunload', () => _sessionUnlocked.delete(fid), { once: true });
}

// ── Unlock with session cache ─────────────────────────────
export async function unlockFolderWithSession(fid) {
  if (isSessionUnlocked(fid)) return true;
  const snap = await get(fRef(fid));
  if (!snap.exists()) return false;
  const folder = snap.val();
  const entered = await import('./utils.js').then(u => u.pinDialog('Unlock Folder', `Enter PIN for "${folder.name}"`));
  if (!entered) return false;
  if (entered !== folder.pin) { (await import('./utils.js')).toast('Wrong PIN', 'error'); return false; }
  setSessionUnlocked(fid);
  return true;
}

// ── Rename folder (also updates history references) ───────
export async function renameFolderAndSync(fid, newName) {
  if (!newName?.trim()) return;
  await update(fRef(fid), { name: newName.trim(), updatedAt: Date.now() });
  // Sync history items that reference this folder
  const histRef = ref(db, `users/${uid()}/history`);
  const snap    = await get(histRef);
  if (snap.exists()) {
    const items = snap.val();
    for (const [key, item] of Object.entries(items)) {
      if (item.folderId === fid) {
