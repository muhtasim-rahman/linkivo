...
export function showImportModal(folders, onImport) {
  ...
  bd.innerHTML = `
    <div class="import-modal" id="import-modal">
      ...
      <!-- Tab pills — Paste merged into File tab -->
      <div class="import-tabs" id="import-tabs">
        ...
        <button class="import-tab" data-tab="url"><i class="fa-solid fa-link"></i> Single URL</button>
      </div>

      <!-- Scrollable body -->
      <div class="import-body" id="import-body">

        <!-- ── FILE TAB (includes paste section below) ─── -->
        <div id="tab-file" class="import-tab-content active">
          ...
          <!-- ── PASTE SECTION (merged into file tab) ─── -->
          <div class="import-paste-section" id="import-paste-section">
            <div class="import-paste-label">
              <i class="fa-solid fa-keyboard" style="color:var(--text-muted)"></i>
              Or paste text / URLs
            </div>
            <div class="import-paste-wrap">
              <textarea id="import-paste" class="import-paste"
                placeholder="Paste URLs, HTML, JSON, Markdown, or any text containing links…" rows="5"></textarea>
              <button class="btn btn-sm import-paste-btn" id="import-clipboard-btn" title="Paste from clipboard">
  ...
  // ── Close / hide ───────────────────────────────────────
  // X button / Cancel: confirm if links detected (state preserved on hide)
  // Backdrop click: just hide (state preserved) — re-open restores it
  const destroy = () => {
  ...
  const hide = () => {
    ...
    // keep bd in DOM so state is preserved
  };
  const closeOrConfirm = async () => {
    if (extracted.length > 0) {
      const { confirm: uiConfirm } = await import('./utils.js');
      const ok = await uiConfirm('Discard import?',
        `You have ${extracted.length} links detected. Discard and close?`, true);
      if (ok) { extracted = []; destroy(); }
    } else {
      destroy();
    }
  };
  $('import-close').onclick  = closeOrConfirm;
  $('import-cancel').onclick = closeOrConfirm;
  // Backdrop click → just hide, state preserved
