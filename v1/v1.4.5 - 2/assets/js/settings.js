  ...
  <div class="st-inner">
      ...
      <div class="st-card">
        ...
        <div class="st-row">
          ...
          <div class="st-row-info">
            ...
            <div class="st-row-sub" id="lock-status">${hasPin?'PIN is set (4-digit) · tap to change or remove':'Protect the app with a 4-digit PIN'}</div>
          </div>
        ...
        <div class="st-row" style="border-top:1px solid var(--border)">
          ...
          <select class="st-select" id="st-auto-lock">
            <option value="0"   ${Storage.get('autoLockMin',0)===0  ?'selected':''}>Immediately (manual only)</option>
            <option value="1"   ${Storage.get('autoLockMin',0)===1  ?'selected':''}>1 min</option>
            <option value="5"   ${Storage.get('autoLockMin',0)===5  ?'selected':''}>5 min</option>
            <option value="10"  ${Storage.get('autoLockMin',0)===10 ?'selected':''}>10 min</option>
            <option value="30"  ${Storage.get('autoLockMin',0)===30 ?'selected':''}>30 min</option>
            <option value="60"  ${Storage.get('autoLockMin',0)===60 ?'selected':''}>1 hour</option>
          </select>
        </div>
        <!-- Forget PIN -->
        <div class="st-row" style="border-top:1px solid var(--border)" id="st-forget-pin-row">
          <div class="st-row-icon" style="background:rgba(245,158,11,0.12);color:var(--warning)"><i class="fa-solid fa-circle-question"></i></div>
          <div class="st-row-info"><div class="st-row-label">Forgot PIN?</div><div class="st-row-sub">Reset via email</div></div>
          <button class="btn btn-sm btn-ghost" id="st-forget-pin">Reset</button>
        </div>
        <!-- Password options -->
        ${isPw ? `
    ...
    <div class="st-section">
      ...
      <div class="st-card">
        ...
        <a class="st-row st-row-btn" href="https://linkivo.web.app/docs" style="border-top:1px solid var(--border);text-decoration:none" target="_blank">
    ...
    <div class="st-section">
      ...
      <div class="st-card">
        ...
        <div class="st-info-row"><span class="st-info-label">Version</span><span class="st-info-value" data-app-version>${cfg.version||'v1.4.5'}</span></div>
        <div class="st-info-row"><span class="st-info-label">Contact</span><a class="st-info-value" href="mailto:linkivo.web@gmail.com" style="color:var(--primary)">linkivo.web@gmail.com</a></div>
        ...
        <div class="st-links-row">
          <a href="https://linkivo.web.app/privacy" target="_blank">Privacy Policy</a>
          <span style="color:var(--border-2)">·</span>
          <a href="https://linkivo.web.app/terms" target="_blank">Terms of Service</a>
          <span style="color:var(--border-2)">·</span>
          <a href="https://linkivo.web.app/docs" target="_blank">Documentation</a>
        </div>
      </div>
    </div>

    <div class="st-footer">Made with <i class="fa-solid fa-heart"></i> · ${cfg.name||'Linkivo'} ${cfg.version||'v1.4.5'}</div>
...
function _bind(user, isPw, isGoogle) {
  ...
  // ── App Lock PIN (4-digit, Firebase synced) ─────────────
  const _saveAppPin = async (pin) => {
    Storage.set('appLockPin', pin);
    Storage.set('lastActiveTs', Date.now());
    const u = getCurrentUser();
    if (u) { try { await update(ref(db, `users/${u.uid}/settings`), { appPin: pin }); } catch {} }
  };
  const _removeAppPin = async () => {
    Storage.remove('appLockPin');
    Storage.remove('lastActiveTs');
    const u = getCurrentUser();
    if (u) { try { await update(ref(db, `users/${u.uid}/settings`), { appPin: null }); } catch {} }
  };

  $('st-lock-btn')?.addEventListener('click', async () => {
    const cur = Storage.get('appLockPin');
    if (cur) {
      const v = await pinDialog('Verify Current PIN', 'Enter your current 4-digit PIN', 4);
      if (!v || v !== cur) { toast('Wrong PIN', 'error'); return; }
      const ch = await confirm('App Lock PIN', 'Change PIN or remove app lock?', false);
      if (!ch) {
        await _removeAppPin();
        document.getElementById('topbar-lock-btn')?.classList.add('hidden');
        $('lock-status').textContent = 'Protect the app with a 4-digit PIN';
        $('st-lock-btn').textContent  = 'Set PIN';
        $('st-lock-btn').className    = 'btn btn-sm btn-primary';
        toast('App lock removed', 'info'); return;
      }
    }
    const p1 = await pinDialog('Set App Lock PIN', 'Choose a 4-digit PIN', 4);
    if (!p1) return;
    const p2 = await pinDialog('Confirm PIN', 'Re-enter the PIN to confirm', 4);
    if (!p2 || p1 !== p2) { toast('PINs do not match', 'error'); return; }
    await _saveAppPin(p1);
    document.getElementById('topbar-lock-btn')?.classList.remove('hidden');
    $('lock-status').textContent = 'PIN is set (4-digit) · tap to change or remove';
    $('st-lock-btn').textContent  = 'Change';
    $('st-lock-btn').className    = 'btn btn-sm btn-secondary';
    toast('App lock PIN set', 'success');
  });

  // Auto-lock timer — save to Firebase too
  $('st-auto-lock')?.addEventListener('change', async e => {
    const v = Number(e.target.value);
    Storage.set('autoLockMin', v);
    const u = getCurrentUser();
    if (u) { try { await update(ref(db, `users/${u.uid}/settings`), { autoLockMin: v }); } catch {} }
  });

  // Forget PIN
  $('st-forget-pin')?.addEventListener('click', async () => {
    const u = getCurrentUser();
    if (!u) { toast('Not signed in', 'error'); return; }

    // Ask which PIN to reset
    const { showModal } = await import('./utils.js');
    const choice = await new Promise(res => {
      const { modal, close } = showModal(`
        <div class="modal-header"><span class="modal-title">Forgot PIN</span>
          <button class="btn btn-ghost btn-icon modal-close-btn"><i class="fa-solid fa-xmark"></i></button></div>
        <div class="modal-body" style="display:flex;flex-direction:column;gap:10px">
          <p style="font-size:var(--fs-sm);color:var(--text-muted)">Which PIN do you need to reset?</p>
          <button class="btn btn-secondary w-full" id="fp-app">App PIN (4-digit)</button>
          <button class="btn btn-secondary w-full" id="fp-folder">Folder PIN (6-digit)</button>
        </div>`);
      modal.querySelector('.modal-close-btn').onclick = () => { close(); res(null); };
      modal.querySelector('#fp-app').onclick    = () => { close(); res('app'); };
      modal.querySelector('#fp-folder').onclick = () => { close(); res('folder'); };
    });
    if (!choice) return;

    const pinType = choice === 'app' ? 'App PIN (4-digit)' : 'Folder PIN (6-digit)';
    const cfg = { name: u.displayName||'User', email: u.email||'', uid: u.uid,
      provider: u.providerData?.[0]?.providerId||'password',
      version: 'v1.4.5', time: new Date().toLocaleString(),
      lang: navigator.language, device: navigator.userAgent.slice(0,120) };

    const body = encodeURIComponent(
`Hello Linkivo Support,

I need to reset my PIN. Please help.

── Account Info ──────────────────────────
Name     : ${cfg.name}
Email    : ${cfg.email}
UID      : ${cfg.uid}
Provider : ${cfg.provider === 'google.com' ? 'Google' : 'Email & Password'}
App Ver  : ${cfg.version}
Time     : ${cfg.time}
Language : ${cfg.lang}
Device   : ${cfg.device}
...
──────────────────────────────────────────
Please reset my PIN or advise next steps.

NOTE: If this email ends up in spam, please check your spam folder.

Thank you.`);
    const sub = encodeURIComponent(`PIN Reset Request — Linkivo (${pinType})`);
    window.open(`mailto:linkivo.web@gmail.com?subject=${sub}&body=${body}`, '_blank');
    toast('Email draft opened — check spam if not received within a few minutes', 'info', 6000);
  });
  ...
  $('st-incognito')?.addEventListener('change',    e => { Storage.set('incognitoMode', e.target.checked); _db('incognitoMode', e.target.checked); });
  ...
  $('st-daily-reminder')?.addEventListener('change', e => { Storage.set('dailyReminder', e.target.checked); _db('dailyReminder', e.target.checked); });
  $('st-import-notif')?.addEventListener('change',   e => { Storage.set('importNotif', e.target.checked); _db('importNotif', e.target.checked); });
  ...
  $('st-auto-clear')?.addEventListener('change', e => { const v=Number(e.target.value); Storage.set('autoClearHistoryDays',v); _db('autoClearHistoryDays',v); });
...
async function _doExport(links, format, selFids, allFolders) {
  ...
  if (format==='json') {
    _dl(JSON.stringify({ exported:new Date().toISOString(), version:'v1.4.5', totalLinks:links.length, folders:groups }, null, 2), `${name}.json`, 'application/json');
    ...
    for (const [folder, items] of Object.entries(groups)) {
      ...
      if (pinned.length) {
        md += `### 📌 Pinned\n\n`;
        pinned.forEach(l => { md += `- [${(l.title||l.domain||l.url).replace(/[\[\]]/g,'')}](${l.url})`; if(l.starred)md+=' [starred]'; if(l.liked)md+=' [liked]'; md+='\n'; });
        md += '\n';
      }
      rest.forEach(l => { md += `- [${(l.title||l.domain||l.url).replace(/[\[\]]/g,'')}](${l.url})`; if(l.starred)md+=' [starred]'; if(l.liked)md+=' [liked]'; md+='\n'; });
      md += '\n';
    }
    ...
    ${Object.entries(groups).map(([folder, items]) => `
      <h2><i class='fa-solid fa-folder' style='margin-right:6px'></i>${escapeHtml(folder)} <span style="font-size:13px;color:#64748b;font-weight:400">(${items.length})</span></h2>
      ${items.map(l => `
        <div class="item">
          ...
          <div style="display:flex;gap:3px">${l.starred?'<span class="badge star"><i class="fa-solid fa-star"></i></span>':''}${l.liked?'<span class="badge like"><i class="fa-solid fa-heart"></i></span>':''}</div>
        </div>`).join('')}`).join('')}
