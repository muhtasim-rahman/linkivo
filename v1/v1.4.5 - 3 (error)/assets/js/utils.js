...
export function pinDialog(title='Enter PIN', message='', {digits=6}={}){
  return new Promise(res=>{
    const dotHtml=[...Array(digits)].map((_,i)=>`<div class="lock-pin-dot" data-i="${i}"></div>`).join('');
    const{modal,close}=showModal(`
      <div class="modal-header">
        <span class="modal-title">${escapeHtml(title)}</span>
        <button class="btn btn-ghost btn-icon modal-close-btn"><i class="fa-solid fa-xmark"></i></button>
      </div>
      <div class="modal-body" style="display:flex;flex-direction:column;gap:16px;align-items:center;padding:24px 20px">
        ${message?`<p style="color:var(--text-muted);font-size:var(--fs-sm);text-align:center;margin:0">${escapeHtml(message)}</p>`:''}
        <div style="display:flex;gap:10px;margin:4px 0">${dotHtml}</div>
        <input
        ...
        <button class="btn btn-ghost btn-sm pin-kbd-btn" style="font-size:12px;color:var(--text-muted)">
    ...
    const dots=modal.querySelectorAll('.lock-pin-dot');
    const errEl=modal.querySelector('.pin-err');

    modal.querySelector('.pin-kbd-btn').onclick=()=>inp.focus();
    inp.focus();
    ...
    function shake(){
      ...
      setTimeout(()=>{inp.value='';syncDots('');dots.forEach(d=>d.classList.remove('error'));inp.focus();},700);
    }
    ...
    inp.addEventListener('input',()=>{
      ...
      syncDots(v);
      errEl.textContent='';
      if(v.length===digits){close();res(v);}  // auto-verify on final digit
    });
...
export async function registerSW(){
  ...
  try{
    ...
    // SW_UPDATED is only sent by the SW on genuine upgrades (old caches existed).
    // On a fresh install the SW does NOT send SW_UPDATED, so no surprise reload.
    navigator.serviceWorker.addEventListener('message',e=>{
      if(e.data?.type==='SW_UPDATED'){
        if(document._swReloading)return;
        document._swReloading=true;
        // Small delay — let any in-flight render finish, then reload
