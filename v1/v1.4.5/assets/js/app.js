...
boot().catch(err => {
  ...
  // If the error looks like a module-load failure (firebase-config.js
  // MIME error or missing file), show the config-error screen.
  const msg = err?.message || String(err);
  const isConfigErr = msg.includes('firebase-config') ||
                      msg.includes('Failed to fetch') ||
                      msg.includes('module') ||
                      msg.includes('MIME');
  if (isConfigErr && typeof window.__showConfigError === 'function') {
    window.__showConfigError();
  } else {
    // Generic crash — hide splash, fall back to auth
    window.__hideSplash?.();
