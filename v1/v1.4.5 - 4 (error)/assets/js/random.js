...
import { toast, Storage, escapeHtml, calcLinkPoints, weightedRandom, copyToClipboard, getDomain } from '/utils.js';
...
// ── Init ──────────────────────────────────────────────────
export function initRandomPage(params = {}) {
  ...
  // Pre-selected folder from folder card random button
  if (params?.preselectedFolderId) {
    const fid = params.preselectedFolderId;
    _selFolders = [fid];
    ...
    setTimeout(() => {
      ...
      if (params.autoOpen) {
        ...
        setTimeout(() => page && _pick(page), 800);
      }
    }, 400);
  }

  // Listen for pre-selected folder from folder 3-dot menu (legacy)
    ...
    <div class="random-shell">
      ...
      <div class="random-header">
        ...
        <div style="display:flex;gap:4px">
          <button class="topbar-btn" id="r-domain-btn" title="Filter by domain">
      ...
      <!-- ── Controls: folders + options (scrolls internally) ── -->
      <div class="random-controls" id="random-controls">

        <!-- Folder selector -->
        <div class="random-controls-inner">
          <div class="r-folder-label">Folders</div>
          <div class="r-folder-bar" id="r-folder-bar"></div>
        </div>

        <!-- Domain filter dropdown panel -->
        <div id="r-domain-panel" class="r-domain-panel hidden">
          <div class="r-domain-panel-head">
            <span class="r-folder-label" style="margin:0">Filter by domain</span>
            <button class="btn btn-ghost btn-sm" id="r-domain-all" style="font-size:11px;padding:2px 8px">All</button>
            <button class="btn btn-ghost btn-sm" id="r-domain-none" style="font-size:11px;padding:2px 8px">None</button>
          </div>
          <div class="r-domain-list" id="r-domain-list"></div>
          <button class="r-domain-more hidden" id="r-domain-more">Show more</button>
        </div>

        <!-- Options panel (collapsible) -->
        <div id="r-options-panel" class="r-options-section hidden">
          <div class="r-opts-title">Options</div>
          ...
          <div class="r-opt-row">
            ...
            <label class="switch">
              <input type="checkbox" id="r-auto" ${_options.autoAdvance?'checked':''}><span class="switch-track"></span>
            </label>
          </div>
          <div class="r-opt-row" id="r-auto-sec-row" ${!_options.autoAdvance?'style="display:none"':''}>
            <div><div class="r-opt-label">Delay</div></div>
      ...
      <!-- ── Preview area: flex:1, always visible ────── -->
      <div class="random-preview-area" id="random-preview-area">
        ...
        <!-- Roll button (shown when no preview active) -->
        <div class="random-main" id="random-main">
        ...
        <div class="r-preview-card hidden" id="r-preview-card">
          ...
          <!-- Preview iframe area (flex:1) -->
          <div class="r-preview-iframe-wrap" id="r-iframe-wrap" style="position:relative;background:var(--surface-2)">

            <!-- Loading overlay -->
            <div id="r-loading" class="r-preview-loading">
            ...
            <div id="r-preview-status" class="r-preview-status hidden"></div>
            ...
            <div id="r-fav-fb" class="r-preview-fallback hidden">
              <img id="r-fav-img" src="" width="56" height="56" style="border-radius:12px;opacity:0.7">
              <div style="font-size:12px;color:var(--text-muted);text-align:center;max-width:200px">Preview unavailable</div>
...
function _bindUI(page) {
  ...
  $('r-opts-btn')?.addEventListener('click', () => {
    ...
    if (!$('r-options-panel')?.classList.contains('hidden')) {
      $('r-domain-panel')?.classList.add('hidden');
    }
  });

  // Domain filter panel toggle
  $('r-domain-btn')?.addEventListener('click', () => {
    $('r-domain-panel')?.classList.toggle('hidden');
    if (!$('r-domain-panel')?.classList.contains('hidden')) {
      $('r-options-panel')?.classList.add('hidden');
    }
  });

  // Domain all / none
  $('r-domain-all')?.addEventListener('click', () => {
    _selDomains = [..._allDomains];
    Storage.set('randomDomains', _selDomains);
    _refreshDomainUI();
  });
  $('r-domain-none')?.addEventListener('click', () => {
    _selDomains = [];
    Storage.set('randomDomains', _selDomains);
    _refreshDomainUI();
  });
...
// ── Domain filter panel ───────────────────────────────────
const DOMAIN_SHOW_LIMIT = 8;
let _domainShowAll = false;

function _refreshDomainUI() {
  const page = document.getElementById('page-random');
  if (!page) return;
  const panel    = page.querySelector('#r-domain-panel');
  const list     = page.querySelector('#r-domain-list');
  const moreBtn  = page.querySelector('#r-domain-more');
  const domainBtn= page.querySelector('#r-domain-btn');

  // Update domain button indicator
  if (domainBtn) {
    const filtered = _selDomains.length > 0 && _selDomains.length < _allDomains.length;
    domainBtn.style.color = filtered ? 'var(--primary)' : '';
    domainBtn.title = filtered
      ? `Domain filter: ${_selDomains.length}/${_allDomains.length}`
      : 'Filter by domain';
  }

  if (!list) return;
  if (_allDomains.length === 0) { list.innerHTML = '<div style="color:var(--text-muted);font-size:12px;padding:4px 0">No domains detected yet — roll a link first</div>'; return; }

  const shown = _domainShowAll ? _allDomains : _allDomains.slice(0, DOMAIN_SHOW_LIMIT);

  list.innerHTML = shown.map(d => {
    const checked = _selDomains.length === 0 || _selDomains.includes(d);
    return `<label class="r-domain-row">
      <input type="checkbox" class="r-domain-chk" data-domain="${escapeHtml(d)}" ${checked?'checked':''}>
      <span class="r-domain-name">${escapeHtml(d)}</span>
    </label>`;
  }).join('');

  list.querySelectorAll('.r-domain-chk').forEach(chk => {
    chk.addEventListener('change', () => {
      const d = chk.dataset.domain;
      if (chk.checked) {
        if (!_selDomains.includes(d)) _selDomains.push(d);
      } else {
        _selDomains = _selDomains.filter(x => x !== d);
      }
      Storage.set('randomDomains', _selDomains);
      _refreshDomainUI();
    });
  ...
  // Show more / less
  if (moreBtn) {
    if (_allDomains.length > DOMAIN_SHOW_LIMIT) {
      moreBtn.classList.remove('hidden');
      if (_domainShowAll) {
        moreBtn.textContent = 'Show less';
        moreBtn.onclick = () => { _domainShowAll = false; _refreshDomainUI(); };
      } else {
        moreBtn.textContent = `Show ${_allDomains.length - DOMAIN_SHOW_LIMIT} more…`;
        moreBtn.onclick = () => { _domainShowAll = true; _refreshDomainUI(); };
      }
    } else {
...
function _pick(page) {
  ...
  // Domain filter (empty _selDomains = all allowed)
  if (_selDomains.length > 0) pool = pool.filter(l => _selDomains.includes(l.domain || getDomain(l.url)));
...
function _showPreview(page, link) {
  ...
  // Status indicator
  const statusEl = page.querySelector('#r-preview-status');
  const setStatus = (msg) => {
    if (!statusEl) return;
    statusEl.textContent = msg;
    statusEl.classList.remove('hidden');
    clearTimeout(statusEl._hideTO);
    if (msg === 'Preview loaded') {
      statusEl._hideTO = setTimeout(() => statusEl.classList.add('hidden'), 2000);
    }
  };
  setStatus('Loading…');

  const hideLoading  = () => loading?.classList.add('hidden');
  const showThumb    = () => { hideLoading(); iframe.classList.add('hidden'); thumbL.classList.remove('hidden'); setStatus('Showing thumbnail'); };
  const showFavLayer = () => {
    hideLoading(); iframe.classList.add('hidden'); thumbL.classList.add('hidden');
    favL.classList.remove('hidden');
    setStatus('Showing favicon');
  ...
  iframe.addEventListener('load', () => { clearTimeout(iframeTO); hideLoading(); setStatus('Preview loaded'); }, { once:true });
