...
export function showModal(html,onClose){
  ...
  // Register with router so hardware back-button closes this modal
  try{ window.Router?.pushModal?.(close); }catch{}
  return{modal,close};
}
...
// pinDialog(title, message, digits=6)
// digits: 4 for app PIN, 6 for folder PIN
// Auto-submits when enough digits entered.
// Prevents Google/1Password from treating as password field.
export function pinDialog(title='Enter PIN', message='', digits=6){
  const dots = Array.from({length:digits},(_,i)=>`<div class="lock-pin-dot" data-i="${i}"></div>`).join('');
  return new Promise(res=>{
    const{modal,close}=showModal(`
      ...
      <div class="modal-body" style="display:flex;flex-direction:column;gap:16px;align-items:center;padding-bottom:8px">
        ${message?`<p style="color:var(--text-muted);font-size:var(--fs-sm);text-align:center;max-width:260px">${escapeHtml(message)}</p>`:''}
        <div class="pin-dots-row">${dots}</div>
        <!-- Attributes prevent Google/1Password password manager prompts -->
        <input class="pin-inp" type="tel" inputmode="numeric"
          maxlength="${digits}" pattern="[0-9]*"
        ...
        <div style="font-size:11px;color:var(--text-subtle)">
          <i class="fa-solid fa-keyboard" style="margin-right:4px"></i>
          Tap the dots to enter PIN
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary btn-sm pin-c">Cancel</button>
        <button class="btn btn-primary btn-sm pin-o">Confirm</button>
      </div>`);

    const inp  = modal.querySelector('.pin-inp');
    const dots_els = modal.querySelectorAll('.lock-pin-dot');
    const err  = modal.querySelector('.pin-err');

    // Make dots tappable / show numeric keyboard on mobile
    modal.querySelector('.pin-dots-row')?.addEventListener('click', () => inp.focus());
    inp.focus();

    const updateDots = (v) => {
      dots_els.forEach((d,i) => {
    ...
    const submit = () => {
      ...
      if (v.length !== digits) {
        err.textContent = `PIN must be ${digits} digits`;
        dots_els.forEach(d => d.classList.add('error'));
        setTimeout(() => { dots_els.forEach(d => d.classList.remove('error')); err.textContent=''; }, 700);
        inp.focus();
        return;
      }
      close(); res(v);
    ...
    inp.addEventListener('input', () => {
      ...
      if (v.length === digits) setTimeout(submit, 120);
    });

    modal.querySelector('.pin-o').onclick = submit;
...
// ══ IMGBB UPLOAD (via Cloudflare Worker proxy) ═══════════
const IMGBB_PROXY = 'https://linkivo.programs-turzo.workers.dev/';

export async function uploadToImgbb(base64Data, name='linkivo-upload') {
  const b64 = base64Data.replace(/^data:[^;]+;base64,/, '');
  const fd  = new FormData();
  fd.append('image', b64);
  fd.append('name',  name);
  const r = await fetch(IMGBB_PROXY, { method:'POST', body:fd });
  if (!r.ok) throw new Error(`ImgBB proxy ${r.status}`);
  const data = await r.json();
  if (!data?.data?.url) throw new Error('ImgBB: no URL');
  return data.data.url;
}

export function compressImage(file, { maxDim=256, quality=0.82 }={}) {
...
export async function registerSW(){
  ...
  try{
    // updateViaCache:'none' ensures sw.js is always fetched fresh from network
    const reg=await navigator.serviceWorker.register('/sw.js',{updateViaCache:'none'});

    // SW_UPDATED is sent by the new SW only on genuine upgrades
    // (i.e. old caches were found and deleted). Fresh installs don't
    // reload — prevents surprise reload on first visit.
    ...
    // skipWaiting is already called in sw.js install, but this handles edge cases.
    const activateSW=(sw)=>{
      if(!sw)return;
      const go=()=>sw.postMessage({type:'SKIP_WAITING'});
      if(sw.state==='installed'){ go(); return; }
      sw.addEventListener('statechange',()=>{ if(sw.state==='installed') go(); });
    };
    if(reg.waiting) activateSW(reg.waiting);
    reg.addEventListener('updatefound',()=>activateSW(reg.installing));

    // Check for updates every 5 minutes (not 30s — less aggressive, less network)
