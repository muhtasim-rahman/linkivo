...
import {
  ...
  calcLinkPoints, Storage, getDomain, getFavicon
} from './utils.js';
...
let _multi      = false;
let _selLinkDomains = []; // domain filter for folder link view
...
export async function addToHistory(link) {
  // Skip if opening from history page (to avoid duplicate entries)
  if (window.__skipHistoryTracking) return;
  const id = genId('h_');
  ...
  return `
    ...
    <div class="links-controls" id="links-controls">
      ...
      <button class="btn btn-ghost btn-sm" id="domain-filter-btn" title="Filter by domain" style="gap:4px">
        ...
        <span id="domain-filter-label" style="max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">Domain</span>
      </button>
...
function _bind(ct, folder) {
  ...
  $('domain-filter-btn')?.addEventListener('click', e => {
    e.stopPropagation();
    _showDomainFilterDropdown($('domain-filter-btn'), _links, ct, folder);
  });
  ...
  $('sort-btn')?.addEventListener('click', e => {
    ...
    showDropdown(e.currentTarget, opts.map(o => ({
      ...
      action: () => { _sort = o.value; Storage.set('linkSort', o.value); _syncSettingToFirebase('linkSort', o.value); if ($('sort-label')) $('sort-label').textContent = o.label; _render(ct, folder); }
    })));
  });
  ...
  ct.querySelectorAll('.view-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      _view = btn.dataset.v; Storage.set('linkView', _view); _syncSettingToFirebase('linkView', _view);
...
function _getList() {
  ...
  // Domain filter
  if (_selLinkDomains.length > 0) {
    list = list.filter(l => _selLinkDomains.includes(l.domain || ''));
  }
  const sorters = {
...
function _listCard(link, folder, idx) {
  ...
  el.innerHTML = `
    ...
    <div class="link-thumb${link.thumbnailUrl?' link-thumb-wide':''}">
      ${link.starred ? '<div class="link-star-dot"></div>' : ''}
      ${link.thumbnailUrl
        ? `<img src="${link.thumbnailUrl}" loading="lazy" class="link-thumb-img" onerror="this.closest('.link-thumb').classList.remove('link-thumb-wide');this.src='${link.favicon||''}';">`
        : `<img src="${link.favicon||''}" loading="lazy" width="32" height="32" style="border-radius:6px" onerror="this.style.display='none'">`
      }
    </div>
    <div class="link-info">
      ...
      <div class="link-meta">
        ...
        ${link.openCount ? `<span><i class='fa-solid fa-eye' style='font-size:9px'></i> ${link.openCount}</span>` : ''}
...
function _gridCard(link, folder, idx) {
  ...
  // Thumbnail: og:image if available, else favicon
  const thumbHtml = link.thumbnailUrl
    ? `<div class="grid-thumb">
        <img src="${link.thumbnailUrl}" loading="lazy" class="grid-thumb-img"
          onerror="this.closest('.grid-thumb').classList.add('grid-thumb-fallback');this.src='${link.favicon||''}';">
      </div>`
    : `<div class="grid-thumb grid-thumb-fallback">
        <img src="${link.favicon||''}" loading="lazy" width="36" height="36"
          style="border-radius:8px;object-fit:contain"
          onerror="this.style.display='none'">
      </div>`;

  el.innerHTML = `
    ${_multi ? `<input type="checkbox" class="grid-cb" ${sel?'checked':''}
      style="position:absolute;top:8px;left:8px;z-index:1;accent-color:var(--primary)">` : ''}
    ${thumbHtml}
    <div class="grid-body">
...
// ── Domain filter dropdown for folder link view ──────────
function _showDomainFilterDropdown(anchor, links, ct, folder) {
  document.querySelectorAll('.domain-filter-dropdown').forEach(d => d.remove());

  const allDomains = [...new Set(links.map(l => l.domain || getDomain(l.url)).filter(Boolean))].sort();
  if (!allDomains.length) { toast('No domains detected', 'info'); return; }

  const dd = document.createElement('div');
  dd.className = 'dropdown domain-filter-dropdown';
  dd.style.cssText = 'min-width:200px;max-height:320px;overflow-y:auto;padding:8px 0';

  // All / None buttons
  dd.innerHTML = `
    <div style="display:flex;gap:6px;padding:4px 12px 8px;border-bottom:1px solid var(--border)">
      <button class="btn btn-ghost btn-sm dfd-all" style="font-size:11px;flex:1">All</button>
      <button class="btn btn-ghost btn-sm dfd-none" style="font-size:11px;flex:1">None</button>
    </div>
    <div class="dfd-list">
      ${allDomains.map(d => {
        const checked = _selLinkDomains.length === 0 || _selLinkDomains.includes(d);
        return `<label class="dropdown-item" style="display:flex;align-items:center;gap:8px;padding:6px 12px;cursor:pointer">
          <input type="checkbox" class="dfd-chk" data-domain="${escapeHtml(d)}" ${checked?'checked':''} style="accent-color:var(--primary)">
          <span style="font-size:13px;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(d)}</span>
        </label>`;
      }).join('')}
    </div>`;
  ...
  const applyFilter = () => {
    const checked = [...dd.querySelectorAll('.dfd-chk:checked')].map(c => c.dataset.domain);
    _selLinkDomains = checked.length === allDomains.length ? [] : checked;
    // Update button label
    const labelEl = document.getElementById('domain-filter-label');
    if (labelEl) {
      labelEl.textContent = _selLinkDomains.length === 0
        ? 'Domain'
        : _selLinkDomains.length === 1 ? _selLinkDomains[0] : `${_selLinkDomains.length} domains`;
    }
    _render(ct, folder);
  };

  dd.querySelector('.dfd-all')?.addEventListener('click', () => {
    dd.querySelectorAll('.dfd-chk').forEach(c => c.checked = true);
    _selLinkDomains = [];
    applyFilter(); dd.remove();
  });
  dd.querySelector('.dfd-none')?.addEventListener('click', () => {
    dd.querySelectorAll('.dfd-chk').forEach(c => c.checked = false);
    _selLinkDomains = allDomains;
    applyFilter(); dd.remove();
  });
  dd.querySelectorAll('.dfd-chk').forEach(chk => {
    chk.addEventListener('change', () => { applyFilter(); });
  });

  const close = e => { if (!dd.contains(e.target) && e.target !== anchor) { dd.remove(); document.removeEventListener('click', close, true); } };
...
// ══════════════════════════════════════════════════════════
// RECYCLE BIN — Full Page Module  (v1.4.5)
// ══════════════════════════════════════════════════════════
export const RecycleBinModule = {
  ...
  init() {
    const page = document.getElementById('page-recycle-bin');
    if (!page) return;
    this._sel.clear();
    this._query = '';
    this._render(page);
    this._subscribe(page);
  },
  ...
  _render(page) {
    page.innerHTML = `
      <div class="rb-page-shell">
        <!-- Header -->
        <div class="rb-header">
          <h2 class="rb-title"><i class="fa-solid fa-trash-can"></i> Recycle Bin</h2>
          <button class="btn btn-danger btn-sm" id="rb-empty-all">
            <i class="fa-solid fa-trash"></i> Empty All
          </button>
        </div>
        ...
        <div class="rb-search-wrap">
          ...
          <input type="text" id="rb-search" placeholder="Search…" autocomplete="off">
        </div>

        <!-- Bulk toolbar -->
        <div class="rb-bulk-bar hidden" id="rb-bulk-bar">
          <span id="rb-sel-count">0 selected</span>
          <button class="btn btn-secondary btn-sm" id="rb-bulk-restore">
            <i class="fa-solid fa-rotate-left"></i> Restore
          </button>
          <button class="btn btn-danger btn-sm" id="rb-bulk-delete">
            <i class="fa-solid fa-trash"></i> Delete
          </button>
          <button class="btn btn-ghost btn-sm" id="rb-desel-all">Deselect all</button>
        </div>
    ...
    this._bindHeader(page);
  },

  _subscribe(page) {
    if (this._unsub) this._unsub();
    const rbRef = ref(db, `users/${uid()}/recycleBin`);
    this._unsub = onValue(rbRef, snap => {
      this._items = snap.exists()
        ? Object.values(snap.val()).sort((a, b) => b.deletedAt - a.deletedAt)
        : [];
      this._renderList(page);
  ...
  _renderList(page) {
    ...
    const visible = q
      ? this._items.filter(item => {
          const name = (item.data?.name || item.data?.title || item.data?.url || '').toLowerCase();
          return name.includes(q);
        })
      : this._items;

    if (!visible.length) {
      const msg = q ? 'No results match your search' : 'Recycle bin is empty';
      const sub = q ? '' : 'Deleted items are kept for 30 days';
      list.innerHTML = `
    ...
    list.innerHTML = visible.map(item => {
      ...
      const expiryColor = daysLeft <= 3 ? 'var(--danger)' : daysLeft <= 7 ? 'var(--warning,#f59e0b)' : 'var(--text-subtle)';
      const sel = this._sel.has(item.id);
      const subLine = isFolder
        ? `Folder · ${item.data?.linkCount || 0} links`
        : `Link · from ${escapeHtml(item.data?.folderName || 'Unknown folder')}`;

      return `
        <div class="rb-row${sel ? ' rb-row-sel' : ''}" data-rbid="${item.id}" data-type="${item.type}">
          <input type="checkbox" class="rb-chk" data-rbid="${item.id}" ${sel?'checked':''}>
          <div class="rb-row-icon">
            <i class="fa-solid ${isFolder ? 'fa-folder' : 'fa-link'}"
              style="color:var(--${isFolder?'warning':'primary'})"></i>
          </div>
          <div class="rb-row-info">
            <div class="rb-row-name">${name}</div>
          ...
          <div class="rb-row-actions">
            <button class="btn btn-ghost btn-sm rb-restore-btn" data-rbid="${item.id}"
              data-type="${item.type}" title="Restore">
              <i class="fa-solid fa-rotate-left"></i>
            </button>
            <button class="btn btn-ghost btn-sm rb-del-btn" data-rbid="${item.id}"
              style="color:var(--danger)" title="Delete permanently">
              <i class="fa-solid fa-xmark"></i>
            </button>
          </div>
        </div>`;
  ...
  _bindHeader(page) {
    ...
    // Empty all
    $('rb-empty-all')?.addEventListener('click', async () => {
      if (!this._items.length) return;
      const ok = await confirm('Empty Recycle Bin', `Permanently delete all ${this._items.length} items? This cannot be undone.`, true);
      if (!ok) return;
      await remove(ref(db, `users/${uid()}/recycleBin`));
      toast('Recycle bin emptied', 'info');
    });
    ...
    $('rb-bulk-restore')?.addEventListener('click', async () => {
      ...
      for (const id of ids) {
        ...
        if (item) await this._restoreItem(item);
      }
      this._sel.clear();
      this._updateBulkBar(page);
    });

    // Bulk delete
    $('rb-bulk-delete')?.addEventListener('click', async () => {
      const cnt = this._sel.size;
      const ok = await confirm('Delete Permanently', `Delete ${cnt} item${cnt!==1?'s':''}? This cannot be undone.`, true);
      if (!ok) return;
      const ids = [...this._sel];
      for (const id of ids) {
        await remove(ref(db, `users/${uid()}/recycleBin/${id}`));
      }
      this._sel.clear();
      this._updateBulkBar(page);
      toast(`${cnt} item${cnt!==1?'s':''} permanently deleted`, 'info');
    });

    // Deselect all
    $('rb-desel-all')?.addEventListener('click', () => {
      this._sel.clear();
      this._updateBulkBar(page);
      this._renderList(page);
    });
  ...
  _bindList(list, page) {
    // Checkbox selection
    list.querySelectorAll('.rb-chk').forEach(chk => {
      chk.addEventListener('change', e => {
        e.stopPropagation();
        const id = chk.dataset.rbid;
        if (chk.checked) this._sel.add(id);
        else             this._sel.delete(id);
        chk.closest('.rb-row')?.classList.toggle('rb-row-sel', chk.checked);
        this._updateBulkBar(page);
      });
    });
    ...
    list.querySelectorAll('.rb-restore-btn').forEach(btn => {
      btn.addEventListener('click', async e => {
        e.stopPropagation();
        const item = this._items.find(i => i.id === btn.dataset.rbid);
        if (item) await this._restoreItem(item);
      });
    });

    // Permanent delete buttons
    list.querySelectorAll('.rb-del-btn').forEach(btn => {
      btn.addEventListener('click', async e => {
        e.stopPropagation();
        const ok = await confirm('Delete Permanently', 'This cannot be undone.', true);
        if (!ok) return;
        await remove(ref(db, `users/${uid()}/recycleBin/${btn.dataset.rbid}`));
  ...
  async _restoreItem(item) {
    try {
      if (item.type === 'folder') {
        ...
        await remove(ref(db, `users/${uid()}/recycleBin/${item.id}`));
        const { incrementLinkCount } = await import('./folders.js');
        await incrementLinkCount(originalFid, 1);
  ...
  _updateBulkBar(page) {
    ...
    const count = page.querySelector('#rb-sel-count');
    if (!bar) return;
  ...
  _updateSidebarCount() {
    ...
    // Sidebar badge
    const badge = document.getElementById('sidebar-recycle-count');
    if (badge) {
      badge.textContent = n;
      badge.classList.toggle('hidden', n === 0);
      // Hide count when sidebar is collapsed (plan says so)
      const sb = document.getElementById('sidebar');
      if (sb?.classList.contains('collapsed')) badge.classList.add('hidden');
    }
    // Home page count
    const rc = document.getElementById('recycle-count');
    if (rc) { rc.textContent = n; rc.classList.toggle('hidden', n === 0); }
  },
...
// ! Removed duplicate showRecycleBin export — was causing SyntaxError.
// The real showRecycleBin() is defined above (~line 884).
// Callers that need router navigation should use: window.Router?.go?.('recycle-bin')