...
// ── Helper: avoids nested backtick ternary (SyntaxError fix) ─
function _buildProviderRow(isPw, isGoogle, safeEmail) {
  if (isPw) {
    return `<div class="st-row st-row-btn" id="st-reset-pw" style="border-top:1px solid var(--border)">
      <div class="st-row-icon" style="background:rgba(59,130,246,0.12);color:var(--primary)"><i class="fa-solid fa-key"></i></div>
      <div class="st-row-info"><div class="st-row-label">Reset Password</div><div class="st-row-sub">Send a reset link to ${safeEmail}</div></div>
      <i class="fa-solid fa-chevron-right st-row-arrow"></i>
    </div>`;
  }
  if (isGoogle) {
    return `<div class="st-row" style="border-top:1px solid var(--border)">
      ...
      <div class="st-card">
        ...
        <div class="st-row">
          ...
          <div class="st-row-info">
            ...
            <div class="st-row-sub" id="lock-status">${hasPin?'PIN is set · 4-digit · in reload grace period':'Lock the entire app with a 4-digit PIN'}</div>
          </div>
        ...
        <div class="st-row" style="border-top:1px solid var(--border)">
          ...
          <select class="st-select" id="st-auto-lock">
            ...
            <option value="-1" ${Storage.get('autoLockMin',0)===-1?'selected':''}>Immediately</option>
            <option value="1"  ${Storage.get('autoLockMin',0)===1?'selected':''}>After 1 minute</option>
            <option value="5"  ${Storage.get('autoLockMin',0)===5?'selected':''}>After 5 minutes</option>
            <option value="10" ${Storage.get('autoLockMin',0)===10?'selected':''}>After 10 minutes</option>
            <option value="30" ${Storage.get('autoLockMin',0)===30?'selected':''}>After 30 minutes</option>
            <option value="60" ${Storage.get('autoLockMin',0)===60?'selected':''}>After 1 hour</option>
          </select>
        </div>
        <!-- Forgot PIN -->
        <div class="st-row st-row-btn" id="st-forgot-pin" style="border-top:1px solid var(--border)">
          <div class="st-row-icon" style="background:rgba(245,158,11,0.12);color:var(--warning,#f59e0b)"><i class="fa-solid fa-circle-question"></i></div>
          <div class="st-row-info"><div class="st-row-label">Forgot PIN?</div><div class="st-row-sub">Send a reset request via email</div></div>
          <i class="fa-solid fa-chevron-right st-row-arrow"></i>
        </div>
        <!-- Password options -->
        ${_buildProviderRow(isPw, isGoogle, escapeHtml(email))}
      </div>
    </div>
...
function _bind(user, isPw, isGoogle) {
  ...
  $('st-lock-btn')?.addEventListener('click', async () => {
    ...
    $('lock-status').textContent = 'PIN is set · 4-digit · in reload grace period';
    $('st-lock-btn').textContent  = 'Change';
    $('st-lock-btn').className    = 'btn btn-sm btn-secondary';
    toast('App lock PIN set <i class="fa-solid fa-lock"></i>', 'success');
  });

  // Auto-lock timer
  $('st-auto-lock')?.addEventListener('change', e => { const v=Number(e.target.value); Storage.set('autoLockMin',v); _db('autoLockMin',v); });
  $('st-forgot-pin')?.addEventListener('click', () => _showForgotPinModal(user));
...
async function _doExport(links, format, selFids, allFolders) {
  ...
  if (format==='json') {
    ...
    for (const [folder, items] of Object.entries(groups)) {
      md += `## <i class="fa-solid fa-folder"></i> ${folder} (${items.length})\n\n`;
      const pinned = items.filter(l=>l.pinned);
      const rest   = items.filter(l=>!l.pinned);
      if (pinned.length) {
        md += `### <i class="fa-solid fa-thumbtack"></i> Pinned\n\n`;
        pinned.forEach(l => { md += `- [${(l.title||l.domain||l.url).replace(/[\[\]]/g,'')}](${l.url})`; if(l.starred)md+=' <i class="fa-solid fa-star"></i>'; if(l.liked)md+=' <i class="fa-solid fa-heart"></i>'; md+='\n'; });
        md += '\n';
      }
      rest.forEach(l => { md += `- [${(l.title||l.domain||l.url).replace(/[\[\]]/g,'')}](${l.url})`; if(l.starred)md+=' <i class="fa-solid fa-star"></i>'; if(l.liked)md+=' <i class="fa-solid fa-heart"></i>'; md+='\n'; });
      md += '\n';
    }
    ...
    ${Object.entries(groups).map(([folder, items]) => `
      <h2><i class="fa-solid fa-folder"></i> ${escapeHtml(folder)} <span style="font-size:13px;color:#64748b;font-weight:400">(${items.length})</span></h2>
      ${items.map(l => `
        <div class="item">
...
// ── Forgot PIN: mailto flow ───────────────────────────────
async function _showForgotPinModal(user) {
  const cfg = Config.get() || {};
  const{showModal} = await import('./utils.js');
  const{modal, close} = showModal(`
    <div class="modal-header">
      <span class="modal-title"><i class="fa-solid fa-circle-question"></i> Forgot PIN</span>
    ...
    <div class="modal-body" style="display:flex;flex-direction:column;gap:14px">
      <p style="color:var(--text-muted);font-size:var(--fs-sm);margin:0">
        Select which PIN you need to reset, then tap <strong>Open Mail</strong> to send a pre-filled request to Linkivo support.
      </p>
      <div style="display:flex;flex-direction:column;gap:8px">
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:var(--fs-sm)">
          <input type="radio" name="pin-type" value="app" checked> App Lock PIN (4-digit)
        </label>
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:var(--fs-sm)">
          <input type="radio" name="pin-type" value="folder"> Folder PIN (6-digit)
        </label>
      </div>
      <div style="background:var(--surface-2);border-radius:var(--r-md);padding:10px 12px;font-size:11px;color:var(--text-muted)">
        <i class="fa-solid fa-triangle-exclamation" style="color:var(--warning,#f59e0b)"></i>
        Emails sent from web apps may land in spam — check your spam folder if you don't receive a reply.
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary btn-sm" id="fp-cancel">Cancel</button>
      <button class="btn btn-primary btn-sm" id="fp-open">
        <i class="fa-solid fa-envelope"></i> Open Mail
      </button>
    </div>`);

  modal.querySelector('.modal-close-btn').onclick = close;
  modal.querySelector('#fp-cancel').onclick = close;
  modal.querySelector('#fp-open').onclick = () => {
    const type = modal.querySelector('input[name="pin-type"]:checked')?.value || 'app';
    const label = type === 'app' ? 'App PIN (4-digit)' : 'Folder PIN (6-digit)';
    const prov = user.providerData?.[0]?.providerId || 'password';
    const provLabel = prov === 'google.com' ? 'Google' : 'Email & Password';

    const subject = 'PIN Reset Request — Linkivo App';
    const body = `Hello Linkivo Support,

I need to reset my PIN. My account details are below:
...
UID         : ${user.uid || 'N/A'}
Provider    : ${provLabel}
App Version : ${cfg.version || 'v1.4.5'}
