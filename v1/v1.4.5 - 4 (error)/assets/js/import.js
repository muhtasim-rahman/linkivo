...
// Module-level state: preserved between modal open/close
let _savedPasteText = '';
let _savedActiveTab = 'file';

export function showImportModal(folders, onImport) {
  if (document.getElementById('import-backdrop')) return; // deduplication guard
    ...
    <div class="import-modal" id="import-modal">
      ...
      <div class="import-body" id="import-body">
        ...
        <div id="tab-paste" class="import-tab-content hidden">
          <!-- Paste from clipboard button + live count row -->
          <div class="import-paste-toolbar">
            <button class="btn btn-secondary btn-sm" id="import-clipboard-btn">
            ...
            <div class="import-live-count hidden" id="import-live-count">
              ...
              <strong id="import-live-num">0</strong> links
              <span id="import-consec-warn" class="hidden" style="color:var(--warning);font-size:11px">
                <i class="fa-solid fa-triangle-exclamation"></i> Consecutive URLs
              </span>
            </div>
          </div>
          <textarea id="import-paste" class="import-paste"
            placeholder="Paste URLs, HTML, JSON, Markdown, or any text with links…"
            rows="6"></textarea>
  ...
  // ── Close ──────────────────────────────────────────────
  // Save state to module-level vars when hiding
  const _saveState = () => {
    _savedPasteText = $('import-paste')?.value || '';
    const activeTab = bd.querySelector('.import-tab.active')?.dataset.tab || 'file';
    _savedActiveTab = activeTab;
  };

  // Hard close: removes from DOM, clears saved state
  const closeAndDiscard = () => {
    _savedPasteText = '';
    _savedActiveTab = 'file';
    bd.classList.remove('open');
    setTimeout(() => bd.remove(), 250);
  };

  // Soft close: saves state, just hides (can reopen)
  const hideAndSave = () => {
    _saveState();
    bd.classList.remove('open');
    setTimeout(() => bd.remove(), 250);
  ...
  const confirmClose = async () => {
    const pasteText = $('import-paste')?.value?.trim() || '';
    const hasContent = pasteText.length > 0;
    if (hasContent) {
      const { confirm: uiConfirm } = await import('/utils.js');
      const yes = await uiConfirm('Discard Import', 'Discard this import and lose entered URLs?', false);
      if (!yes) return;
    }
    closeAndDiscard();
  };

  $('import-close').onclick  = confirmClose;
  $('import-cancel').onclick = confirmClose;

  // Backdrop click → save state (no confirm), just hide
  bd.addEventListener('click', e => { if (e.target === bd) hideAndSave(); });
  ...
  // ── Restore saved state ─────────────────────────────────
  if (_savedPasteText) {
    const pasteEl = $('import-paste');
    if (pasteEl) {
      pasteEl.value = _savedPasteText;
      pasteEl.dispatchEvent(new Event('input')); // trigger live count
    }
  }
  if (_savedActiveTab && _savedActiveTab !== 'file') {
    const tab = bd.querySelector(`.import-tab[data-tab="${_savedActiveTab}"]`);
    if (tab) tab.click();
  }
  ...
  $('import-clipboard-btn')?.addEventListener('click', async () => {
    try {
      ...
      const pasteEl = $('import-paste');
      if (!pasteEl) return;
      pasteEl.value = (pasteEl.value ? pasteEl.value + '\n' : '') + text;
      pasteEl.dispatchEvent(new Event('input')); // trigger live URL count
      pasteEl.scrollTop = pasteEl.scrollHeight;
    } catch {
      // Clipboard access denied — show hint
      const pasteEl = $('import-paste');
