...
// ── Upload icon to imgbb via Cloudflare Worker ─────────────
async function _uploadIcon(imageFile, contextName = 'folder') {
  try {
    const u    = getCurrentUser();
    const uid2 = u?.uid || 'anon';
    const safe = (contextName || 'folder').replace(/[^a-z0-9]/gi, '_').toLowerCase().slice(0, 20);
    const name = `linkivo_folder_icon_${uid2}_${safe}_${Date.now()}`;
    const fd   = new FormData();
    fd.append('image', imageFile, name + '.jpg');
    fd.append('name', name);
    const res  = await fetch('https://linkivo.programs-turzo.workers.dev/', { method: 'POST', body: fd });
...
async function _showIconPicker(folder) {
  if (document.getElementById('icon-picker-backdrop')) return;
  const ICONS = [
    'fa-solid fa-folder','fa-solid fa-bookmark','fa-solid fa-star',
    'fa-solid fa-gamepad','fa-solid fa-briefcase','fa-solid fa-music',
    'fa-solid fa-book','fa-solid fa-palette','fa-solid fa-flask',
  ...
  const bd = document.createElement('div');
  bd.id = 'icon-picker-backdrop';
  bd.className = 'modal-backdrop';
  bd.innerHTML = `
    <div class="modal" style="max-width:340px">
      <div class="modal-header">
        <span class="modal-title"><i class="fa-solid fa-icons"></i> Change Icon</span>
        <button class="btn btn-ghost btn-icon ip-close"><i class="fa-solid fa-xmark"></i></button>
      </div>
      <div class="modal-body">
        <div class="cf-icon-grid" id="ip-icon-grid">
          ${ICONS.map(ic => `<button class="cf-icon-btn${(folder.icon===ic&&!folder.iconUrl)?' active':''}" data-icon="${ic}">
            <i class="${ic}"></i>
          </button>`).join('')}
        </div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:10px;flex-wrap:wrap">
          <button class="btn btn-secondary btn-sm" id="ip-upload-btn">
            <i class="fa-solid fa-image"></i> Upload image
          </button>
          <input type="file" id="ip-upload-input" accept="image/*" style="display:none">
          <span id="ip-status" style="font-size:11px;color:var(--text-muted)">
            ${folder.iconUrl ? 'Custom image active' : ''}
          </span>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary btn-sm ip-close">Cancel</button>
  ...
  let _selIcon  = folder.icon  || 'fa-solid fa-folder';
  let _newIconUrl = folder.iconUrl || null;
  ...
  $('#ip-upload-btn')?.addEventListener('click', () => $('#ip-upload-input')?.click());
  $('#ip-upload-input')?.addEventListener('change', async e => {
    const file = e.target.files?.[0];
    if (!file) return;
    const statusEl = $('#ip-status');
    statusEl.textContent = 'Uploading…';
    const url = await _uploadIcon(file, folder.name);
    if (url) { _newIconUrl = url; statusEl.textContent = 'Uploaded!'; bd.querySelectorAll('.cf-icon-btn').forEach(x => x.classList.remove('active')); }
    else statusEl.textContent = 'Upload failed';
  });

  const close = () => bd.remove();
  bd.querySelectorAll('.ip-close').forEach(b => b.addEventListener('click', close));
  bd.addEventListener('click', e => { if (e.target === bd) close(); });

  $('#ip-save')?.addEventListener('click', async () => {
    await update(fRef(folder.id), { icon: _selIcon, iconUrl: _newIconUrl, updatedAt: Date.now() });
    close();
