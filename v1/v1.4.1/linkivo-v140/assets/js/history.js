...
import { isSessionUnlocked, unlockFolderWithSession } from './folders.js';
...
function createHistoryItem(item, idx) {
  ...
  // Check if item belongs to a locked folder that isn't session-unlocked
  const isLocked = item.folderId && !isSessionUnlocked(item.folderId);
  const blurClass = isLocked ? 'hist-locked' : '';

  el.innerHTML = `
    <div class="hist-thumb ${blurClass}">
      ...
      ${isLocked ? '<div class="hist-lock-icon"><i class="fa-solid fa-lock"></i></div>' : ''}
    </div>
    <div class="hist-info ${blurClass}">
      <div class="hist-title">${isLocked ? '🔒 Locked Content' : escapeHtml(item.title || item.domain || 'Link')}</div>
      <div class="hist-url">${isLocked ? '••••••••••••••••' : escapeHtml(item.url || '')}</div>
      <div class="hist-meta">
  ...
  el.addEventListener('click', async (e) => {
    ...
    // If locked, ask for PIN first
    if (isLocked) {
      const ok = await unlockFolderWithSession(item.folderId);
      if (!ok) return;
      // Re-render this item without blur
      const newEl = createHistoryItem({ ...item }, idx);
